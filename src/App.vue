<template>
  <div class="m-4 grid grid-auto-fit-[30%]  h-screen items-center space-x-2 space-y-3 ">
    <div>
      ...
      <NModal class=" !w-auto inset-0 z-10 !fixed " :open="openModal" >
        <NModalBg class=" inset-0 justify-center intems-center">
            <NModalContent id="modalContent" class=" bg-white w-[500px]  border-2 border-primary-500 rounded-lg">
    ...
    <div>
      <NCardSummary class=" border-2 border-b-primary-500 border-b-4 border-gray-200" :data="{icon:'fa fa-user', title: 'test', description:'hello'}" />
    </div>
    <div>
      <NTabWrapper active-class="hover:hover:bg-gray-500 bg-primary-500 border-b-4 border-secondary-400" default-class="" :row-align="true" >
    ...
    <div class=" space-y-2">
      ...
      :multipleSelect="true"
      :optionFormat="optionFormat"
      :selectedOptions="[selectOptions[1]]"
      @change=""
      >
        ...
        <template #list="{optionFormat, options}" >
          <NList class=""
              ...
              :options="options"
              :multipleSelect="true"
              :selectedOptions="[selectOptions[3]]"
              @change=""
              />
        </template>
      </NListSearch> -->
...
const imageCropped = (data: string) => {
  preview.value = data;
  //console.log(data);
