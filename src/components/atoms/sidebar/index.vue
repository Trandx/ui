<template>
  <ul class="space-y-2 font-medium">
    <li v-for="(tab, key) in items" :key="key">
      <router-link :to="{ name: tab.name }" custom v-slot="{ isExactActive, href, navigate }">
        <!-- v-slot="{isActive, isExactActive, href, navigate}" -->
        <a
          :href="href"
          @click="navigate"
          :class="`${(checkPath(href) || isExactActive) && active_class.for_item} ${items_class}`"
        >
          <span class="">
            <i
              :class="`${tab?.icon?.name} 
                        ${tab?.icon?.class || ''} 
                        ${(checkPath(href) || isExactActive) && active_class.for_icon}`"
            ></i>
          </span>
          <span :class="`ml-3 ${!props.open && 'hidden'}`"> {{ tab.alias || tab.name }}</span>
        </a>
      </router-link>
    </li>
  </ul>
</template>
<script setup lang="ts">
import { useRoute } from 'vue-router'
import type { ISidebar } from './index.type'

const props = defineProps<ISidebar>()

const currentRoute = useRoute()

const checkPath = (currentPath: string) => {
  if (props.strict) return currentPath == currentRoute.path

  return currentRoute.path.includes(currentPath)
}
</script>
