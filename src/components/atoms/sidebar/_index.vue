  ...
  <ul class="space-y-2 font-medium">
    <li v-for="(tab, key) in datas.items" :key="key">
      <router-link
        ...
        :class="`${datas.current_item == tab.name.toLowerCase() && datas.active_class.for_item} 
        ${datas.items_class}`"
      >
...
const patchmacth = () => {
  ...
  return matchedPatch.matched.some((matchedRoute) => {
    const result = datas.items.find(
      (item) => item.name.toLowerCase() === matchedRoute.name?.toString().toLowerCase(),
    )
