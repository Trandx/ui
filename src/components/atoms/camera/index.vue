<template>
    <div class="border-2 border-primary-500 rounded-lg  grid grid-cols-1 content-between">
        <div class="bg-primary-500 h-full flex justify-between items-center py-0.5 space-x-2 px-1">
            <div class="text-white text-lg flex items-center bg-primary-500 ">
                <i class="fa-regular fa-camera-web"></i>
            </div>
            <NSelect class="w-full ring-1 ring-white rounded-lg" :options="cameras" placeholder="select Camera"
                :multiple-select="false" :autoclose="true" :searchable="true" :selected-options="cameras[0] || null"
            ...
            <div class="text-white text-lg" v-if="showStream">
                <button @click.prevent="toggle"
                    class="px-2 py-1 bg-red-500 hover:bg-red-400 focus:bg-red-400 rounded-lg" title="stop stream">
            ...
            <div class="text-white text-lg border-2 border-white rounded-lg" v-else>
                <button @click.prevent="toggle"
                    class="px-2 py-0.5 bg-primary-500 hover:bg-primary-300 focus:bg-primary-300 rounded-lg"
                    title="play stream">
        ...
        <div class="w-full flex justify-center items-center">
            <div v-if="isLoading">
                <NSpinnerGrow class="w-12 h-12 bg-primary-500" />
            </div>
            ...
            <div class="relative" v-else>
                ...
                <NCropImage ref="cropper" v-else class=" w-full" :src="cropImageSrc" :cropBoxResizable :aspectRatio
                    :cropBoxWidth :cropBoxHeight @ready="" @preview="" @cropmove="" @cropstart="" @cropend="cropend" />
            </div>
        ...
        <div class=" bg-secondary-400 h-[40px] rounded-b-lg  px-2">
            ...
            <div v-if="isPhotoTaken" class="flex flex-wrap justify-between items-center h-full">
                <div class="flex justify-between w-auto space-x-4">
                    <button title="back to cameras"
                        class="bg-gray-400 hover:bg-primary-500 hover:text-white  flex items-center p-1 rounded-full"
                ...
                <div class="flex justify-end space-x-6 px-3">
                    ...
                    <div class="flex justify-center space-x-2 ">
                        <button
                            class="bg-gray-400 hover:bg-primary-500 hover:text-white  flex items-center p-1 rounded-full"
                            @click.prevent.prevent="flipY">
                            <i class="fa-sharp fa-solid fa-reflect-vertical"></i>
                        </button>
                        <button
                            class="bg-gray-400 hover:bg-primary-500 hover:text-white  flex items-center p-1 rounded-full"
                            @click.prevent.prevent="flipX">
                    ...
                    <div class="flex justify-center space-x-2">
                        ...
                        <button
                            class="bg-gray-400 hover:bg-primary-500 hover:text-white  flex items-center px-1.5  rounded-full"
                    ...
                    <div class="flex justify-center space-x-2">
                        <button
                            class="bg-gray-400 hover:bg-primary-500 hover:text-white  flex items-center px-1 rounded-full"
                            @click.prevent="reset">
...
const toggle = async () => {
    ...
    if (!showStream.value) {
        ...
        try {
            ...
            const device = Stream.selectedDevice;

            console.log(device);

            streamData.value = await Stream.startVideoStream()
...
const changeCamera = async (device: VideoDataType) => {
    ...
    try {
        ...
        if (Stream.permission.is_granted) {
            const stream = await Stream.changeCamera(device.value)
...
const refreshCamera = async () => {
    try {
        ...
        // S'assure que isLoading est réinitialisé quel que soit le résultat
        isLoading.value = false;
    }
}
...
const getCameras = async () => {
    try {
        const camerasDevices = await Stream.getCameras();
        ...
        if (camerasDevices) {
            ...
            if (props.autoplay && cameras.value.length > 0) {
                const stream = await Stream.changeCamera(cameras.value[0].value);
