<template>
  ...
  <div
    ref="toastElt"
    class="w-auto min-w-[320px] bg-white rounded-lg shadow-lg flex flex-wrap justify-center item-center"
    role="alert"
  >
...
import type { IToast } from './index.type'
...
const toggleToast = () => {
  if (open.value) {
    ...
    openAnimation
      ? openAnimation(toastElt.value as HTMLElement)
      : zoomIn(toastElt.value as HTMLElement)
  } else {
    const closeAnimation = props.animation?.close

    closeAnimation
      ? closeAnimation(toastElt.value as HTMLElement)
      : zoomOut(toastElt.value as HTMLElement)
  }
}

watch([open, () => props], ([openState, propsState]) => {
