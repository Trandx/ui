<template>
  <div ref="dropdrownElement" v-click-outside="closeToggle" class="relative inline-flex">
    ...
    <div
      ...
      :class="[
        !isOpen && 'hidden',
        positionList?.top ? ' bottom-[100%] mb-0.5' : ' top-[100%] mt-0.5',

        positionList?.left ? 'right-0' : 'left-0',
      ]"
      class="absolute z-10 min-w-full"
      tabindex="0"
    >
      <slot name="items" :options="options" :optionFormat="optionFormat" @change="selectedItem">
        <div
          class="min-w-60 shadow-md py-2 bg-secondary-500 border border-gray-700 divide-gray-700 transition-[opacity,margin,hidden] duration-[50000]"
        >
          {{ options }}
          <a
...
import type { IDropDown } from './index'
