<template>
  ...
  >
    <div
      class="flex justify-center items-center font-medium bg-secondary-500"
      :class="[!open ? 'rounded-lg' : putItemsListToTop ? 'rounded-b-lg' : 'rounded-t-lg']"
      ref="selectElement"
    >
      <div
        :class="[
        ...
        @blur="closeToggle"
      >
        <slot name="optionSelected" :selectedItems="selectedItems">
      ...
      <div class="w-full">
        <input
          type="text"
          @click="!disabled && (open = false)"
          @input="getPhoneNumberInGoodFormat"
          v-model="phoneNumber"
          :maxlength="max"
          :minlength="min"
          placeholder="phone  number"
          :disabled
          class="focus:ring-primary-500 border-none disabled:bg-gray-400 w-full rounded-r-lg py-0 focus:ring-2 pl-2 focus:bg-gray-700 text-gray-50 hover:bg-gray-700 bg-secondary-500 placeholder:italic placeholder:font-light"
        />
      </div>
    </div>
    <div
    ...
    >
      <div class="relative m-2" v-if="searchable || true" @click.stop="!disabled && (open = true)">
        <input
          type="search"
          class="block appearance-none py-0.5 px-1 w-full z-20 text-gray-900 bg-gray-50 rounded-lg border-l-2 border focus:ring-primary-500 ring-primary-500 focus:ring-1 focus:outline-none placeholder:italic placeholder:font-light"
          placeholder="Search"
          @keyup="() => search"
          v-model="searchInput"
          tabindex="0"
        ...
        />
        <span
          :class="`${activeClass} absolute top-0 right-0 p-0.5 font-medium text-white rounded-r-lg border border-primary-500 hover:bg-primary-500 focus:ring-4 focus:outline-none focus:ring-primary-500 `"
        >
...
import type { IPhone } from './index.type'
...
const getPhoneNumberInGoodFormat = () => {
  const data =
    phoneNumber.value &&
    InputRules.parseToNumber({ value: phoneNumber.value, pattern: props.pattern })
