<template>
  <div class="text-white relative bg-secondary-500 divide-gray-800" ref="itemList">
    <slot
      name="search"
    ...
    <slot name="list" :options="options" :optionFormat="optionFormat">
      <NList
...
const doSearch = async (keyword: string) => {
  ...
  const { data: new_data } = await search({
    data: props.options,
