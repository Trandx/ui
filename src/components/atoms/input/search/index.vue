<template>
  <div class="bg-secondary-500 p-1 rounded-lg">
    <div class="relative flex justify-between focus:border-2 focus:border-primary-500">
      <div class="absolute top-0 left-0 h-full" v-if="useMicrophone">
        <slot name="microphoneBtn" :toggleMic="ToggleMic">
          <button
            class="bg-primary-500 px-2 h-full text-white"
            title="run microphone"
            @click="ToggleMic"
          >
      ...
      <div class="w-full">
        ...
        >
          <input
            ...
            @input="handleChange"
            class="block w-full py-1 z-20 text-gray-900 bg-gray-50 border ring-primary-500 focus:outline-none placeholder:italic rounded-lg border-primary-500 focus:ring-1 focus:ring-primary-500"
...
const Recognition = (<any>window).SpeechRecognition || (<any>window).webkitSpeechRecognition
