    ...
    <div :class="`${!rowAlign?'flex':''} space-y-2 `">
        <nav class="isolate shadow w-max">
            <ul :class="`${rowAlign?'flex flex-wrap':'block'} mx-auto gap-x-2`">
                ...
                :class="`${defaultClass || 'hover:bg-gray-500 px-4'} ${ (tabDatail.title == selectedTitle)  ? (activeClass || 'bg-primary-500 text-white font-bold border-b-4 border-secondary-400') : ''}`"
