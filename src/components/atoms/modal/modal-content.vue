<template>
  <div ref="modalContent">
    <div
      class="resizer r_mid_right cursor-col-resize h-full absolute right-0 top-0 w-1 z-[11]"
    ></div>
    ...
    <!-- cornaries positions-->
    <div
      class="resizer r_bottom_right cursor-nwse-resize w-1 absolute bottom-0 right-0 h-2 z-[11]"
    ></div>
    <div
      class="resizer r_bottom_left cursor-nesw-resize w-1 absolute bottom-0 left-0 h-2 z-[11]"
    ></div>
...
let modalBodyScrollHeight: number | null,
  modalBodyScrollWidth: number | null = null
...
function adjustModalBodySiize() {
  ...
  const currentModalBody =
    parentHeight - (modalHeader?.offsetHeight || 0) - (modalFooter?.offsetHeight || 0)
